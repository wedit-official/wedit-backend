...
3. `finish-pr.sh` 는 최신 PR head commit에 대한 Gemini bot review/comment activity가 도착할 때까지 기다립니다.
...
- 새 커밋을 push한 뒤 다시 `finish-pr.sh` 를 실행하면, 이전 Gemini review가 아니라 최신 head commit 이후의 Gemini activity를 기다립니다.
