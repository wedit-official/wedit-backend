...
- [2026 웨딧 백엔드 기능 명세](./specs/2026-wedit-backend-capabilities.md)
- [2026 웨딧 기능 커버리지 매트릭스](./specs/2026-wedit-backend-coverage-matrix.md)
- 현재 실행 계획: `docs/exec-plans/active/`
- 완료된 실행 계획: `docs/exec-plans/completed/`

## 기본 원칙
- 기본 검증 명령은 `./gradlew check build --no-daemon` 입니다.
- 테스트는 `test` profile에서 외부 의존성 없이 통과해야 합니다.
- 제품 기능 명세는 `docs/specs/sources` 원본 export와 `docs/specs/generated` 정규화 산출물로 추적합니다.
- 백엔드 기능 작업은 `feature_id`를 기준으로 `EXEC_PLAN`, 구현, 테스트, 커버리지 매트릭스를 함께 갱신합니다.
- 하네스/인프라 작업은 `EXEC_PLAN`의 `Related Feature IDs`에 `n/a-harness`를 기록합니다.
...
- PR 완료는 `scripts/task/finish-pr.sh <PR_NUMBER>`로 수행해 merge, `develop` 동기화, feature worktree 제거, branch 삭제를 한 번에 끝냅니다.
